...
<!-- UsageSnippet language="typescript" operationID="listClasses" method="get" path="/classes" -->
```typescript
import { PushPress } from "@pushpress/pushpress";
...
async function run() {
  const result = await pushPress.classes.list({});
...
```typescript
import { PushPressCore } from "@pushpress/pushpress/core.js";
import { classesList } from "@pushpress/pushpress/funcs/classesList.js";
...
async function run() {
  const res = await classesList(pushPress, {});
  if (res.ok) {
    const { value: result } = res;
    for await (const page of result) {
    console.log(page);
    ...
    console.log("classesList failed:", res.error);
  }
}
