 ...
 */
export function manageWebhooksDeactivate(
  client: PushPressCore,
  request: operations.DeactivateWebhookRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    operations.DeactivateWebhookResponseBody,
    | APIError
    | SDKValidationError
    | UnexpectedClientError
    | InvalidRequestError
...
> {
  ...
  const path = pathToFunc("/webhooks/{uuid}/deactivate")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(204, operations.DeactivateWebhookResponseBody$inboundSchema),
    M.fail([401, 403, "4XX"]),
    M.fail("5XX"),
  )(response);
  if (!result.ok) {
