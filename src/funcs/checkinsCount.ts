 ...
 */
export function checkinsCount(
  client: PushPressCore,
  request: operations.CountCheckinsRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    operations.CountCheckinsResponseBody,
    | PushPressError
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.CountCheckinsRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/checkins/count")();
  ...
  const query = encodeFormQuery({
    ...
    "includeHistoricalCheckins": payload.includeHistoricalCheckins,
    "type": payload.type,
  });
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.CountCheckinsResponseBody$inboundSchema),
    M.fail([401, "4XX"]),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
