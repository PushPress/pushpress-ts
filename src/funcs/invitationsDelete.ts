 ...
 */
export function invitationsDelete(
  client: PushPressCore,
  request: operations.DeleteInvitationByIdRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    operations.DeleteInvitationByIdResponseBody,
    | PushPressError
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.DeleteInvitationByIdRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/invitations/{id}")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(204, operations.DeleteInvitationByIdResponseBody$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
