...
> {
  const parsed = safeParse(
    request,
    (value) => operations.DeleteAppInstallRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/apps/{app_id}/installs/{install_id}/delete")(
    pathParams,
  );
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(204, components.AppInstall$inboundSchema),
    M.fail(["4XX", "5XX"]),
  )(response);
  if (!result.ok) {
    return result;
