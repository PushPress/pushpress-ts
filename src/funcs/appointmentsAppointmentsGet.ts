 ...
 * Get details for an appointment
 */
export function appointmentsAppointmentsGet(
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.GetAppointmentRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/appts/{id}")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.Appointment$inboundSchema),
    M.fail([404, "4XX"]),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
