 ...
 */
export async function appsInstallsGet(
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.GetAppInstallRequest$outboundSchema.parse(value),
  ...
  const pathParams = {
    ...
    install_id: encodeSimple("install_id", payload.install_id, {
      explode: false,
      charEncoding: "percent",
    }),
  };

  const path = pathToFunc("/apps/{app_id}/installs/{install_id}")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, components.AppInstall$inboundSchema),
    M.fail(["4XX", "5XX"]),
  )(response);
  if (!result.ok) {
    return result;
