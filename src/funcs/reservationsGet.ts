 ...
 */
export function reservationsGet(
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.GetReservationRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/reservations/{id}")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, components.Reservation$inboundSchema),
    M.fail([404, "4XX"]),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
