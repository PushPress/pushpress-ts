 ...
 * Get details for a class
 */
export async function classesGet(
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.GetClassRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/classes/{id}")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, components.Class$inboundSchema),
    M.fail([404, "4XX"]),
    M.fail("5XX"),
  )(response);
  if (!result.ok) {
