...
export async function checkinsList(
  ...
  PageIterator<
    Result<
      operations.ListCheckinsResponse,
      | SDKError
      | SDKValidationError
      | UnexpectedClientError
      | InvalidRequestError
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.ListCheckinsRequest$outboundSchema.parse(value),
  ...
  const [result, raw] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, operations.ListCheckinsResponse$inboundSchema, {
      key: "Result",
    }),
    M.fail(["4XX", "5XX"]),
