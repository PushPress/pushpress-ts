 ...
 */
export async function enrollmentGet(
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.GetEnrollmentRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/enrollments/{uuid}")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, components.Enrollment$inboundSchema),
    M.fail([401, 404, "4XX"]),
    M.fail("5XX"),
  )(response);
  if (!result.ok) {
