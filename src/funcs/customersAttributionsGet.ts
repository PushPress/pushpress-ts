 ...
 */
export async function customersAttributionsGet(
  client: PushPressCore,
  request: operations.GetAttributionRequest,
  options?: RequestOptions,
): Promise<
  Result<
    operations.GetAttributionResponseBody,
    | APIError
    | SDKValidationError
    | UnexpectedClientError
    | InvalidRequestError
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.GetAttributionRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/attributions/attributions/{uuid}")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, operations.GetAttributionResponseBody$inboundSchema),
    M.fail([401, 403, "4XX"]),
    M.fail("5XX"),
  )(response);
  if (!result.ok) {
